#!/usr/bin/env python3
"""
D405 ArUco -> Enter -> Tool (stable 5x ICP)

Ablauf:
1) ArUco wird laufend erkannt und als TF publiziert.
2) Nach erster ArUco-Erkennung wartet der Node auf Enter.
3) Nach Enter werden genau 5 ICP-Runs ausgefuehrt (jeweils auf frischer Cloud).
4) Nur wenn drei erfolgreiche ICP-Runs direkt hintereinander innerhalb Toleranz liegen,
   wird die Pose publiziert.
"""
...
# Stabilitaetslogik
ICP_RUNS_PER_TRIGGER = 5
MIN_RUNS_BEFORE_EARLY_ACCEPT = 3
STABLE_POS_TOL_M = 0.004
STABLE_ANG_TOL_DEG = 6.0
REQUIRED_STABLE_SUCCESS_STREAK = 3

# Zusatztor: Wenn 2 Runs hintereinander sehr stark sind, direkt publizieren.
STRONG_QUALITY_MIN_RUNS = 3
STRONG_FITNESS_MIN = 0.985
STRONG_RMSE_MAX = 0.0040
...
class D405ArucoThenToolStable5Node(Node):
    def __init__(self) -> None:
        super().__init__("d405_aruco_then_tool_stable5_node")
        ...
        # Stabilitaets-Zustaende
        self.icp_runs_done = 0
        self.prev_success_pose = None  # (p_rel, r_rel)
        self.prev_success_run_index = -1
        self.stable_pair_pose = None   # (p_rel, r_rel)
        self.stable_pair_found = False
        self.strong_quality_streak = 0
        ...
        self.get_logger().info("Node gestartet: ArUco -> Enter -> 5x ICP -> stabile Doppel-Pose")
    ...
    def _on_image(self, msg: Image) -> None:
        ...
        for marker_id, rvec, tvec in zip(ids.flatten(), rvecs, tvecs):
            ...
            with self.marker_lock:
                self.marker_position = t.copy()
                self.marker_rotation = np.array(r_mat, dtype=np.float64)

            if not self.aruco_found:
                self.aruco_found = True
                self.wait_for_enter = True
                self.get_logger().info("Aruco erkannt, Enter fuer 5x ICP")
            return

    def _input_loop(self) -> None:
        while not self._shutdown and rclpy.ok():
            if not self.wait_for_enter or self.scan_active:
                time.sleep(0.1)
                continue

            try:
                user = input("\n[Capture] Enter fuer Scan (q zum Beenden): ").strip().lower()
    ...
    def _start_tool_scan_once(self) -> None:
        ...
        self.icp_runs_done = 0
        self.prev_success_pose = None
        self.prev_success_run_index = -1
        self.stable_pair_pose = None
        self.stable_pair_found = False
        self.strong_quality_streak = 0
        self.stable_success_streak = 0
        ...
        self.get_logger().info("Tool-Scan gestartet (frueher Abschluss bei stabilen 2 Runs moeglich)")
    ...
    def _cloud_worker(self) -> None:
        while not self._shutdown and rclpy.ok():
            ...
            if time.time() > self.scan_deadline:
                self.scan_active = False
                self._publish_status("TOOL_SCAN_TIMEOUT")
                self.get_logger().warn(
                    f"Scan-Timeout nach {TOOL_TIMEOUT_SEC:.1f}s ohne stabile Freigabe"
                )
                continue
            ...
            try:
                ...
                self.get_logger().debug(f"Scan Debug: {exc}")
    ...
    def _process_cloud_msg(self, msg: PointCloud2) -> None:
        ...
        self.icp_runs_done += 1
        result, best_fit, best_rmse = self._match_roi_points(roi_points)
        # Timeout nach jedem verarbeiteten Run nach hinten schieben, damit langsame Erst-Runs
        # nicht vorzeitig den gesamten Scan beenden.
        self.scan_deadline = time.time() + TOOL_TIMEOUT_SEC
        self.get_logger().info(
            f"ICP run {self.icp_runs_done}/{ICP_RUNS_PER_TRIGGER}: fit={best_fit:.4f}, rmse={best_rmse:.4f}"
        )
        ...
        if result is not None:
            ...
            if best_fit >= STRONG_FITNESS_MIN and best_rmse <= STRONG_RMSE_MAX:
                self.strong_quality_streak += 1
            else:
                self.strong_quality_streak = 0

            is_consecutive = (self.prev_success_run_index == self.icp_runs_done - 1)
            if self.prev_success_pose is not None and is_consecutive:
                p_prev, r_prev = self.prev_success_pose
                dp = float(np.linalg.norm(p_rel - p_prev))
                da = rot_angle_deg(r_prev, r_rel)
                if dp <= STABLE_POS_TOL_M and da <= STABLE_ANG_TOL_DEG:
                    self.stable_success_streak += 1
                    self.stable_pair_found = True
                    self.stable_pair_pose = (p_rel.copy(), r_rel.copy())
                    self.get_logger().info(
                        f"Stabile Doppel-Pose: dp={dp:.4f}m, da={da:.2f}deg"
                    )
                else:
                    self.stable_success_streak = 1
            else:
            ...
            self.prev_success_pose = (p_rel.copy(), r_rel.copy())
            self.prev_success_run_index = self.icp_runs_done
        else:
            # Ein Fehlschlag unterbricht die Bedingung "nacheinander".
            self.prev_success_pose = None
            self.prev_success_run_index = -1
            self.strong_quality_streak = 0
            self.stable_success_streak = 0
        ...
        if (
            result is not None
            and self.strong_quality_streak >= STRONG_QUALITY_MIN_RUNS
            and self.icp_runs_done >= MIN_RUNS_BEFORE_EARLY_ACCEPT
            and self.stable_success_streak >= REQUIRED_STABLE_SUCCESS_STREAK
        ):
            self.scan_active = False
            self._publish_detected_tf(p_rel, r_rel)
            self._publish_pose(p_rel, r_rel)
            self._publish_status("TOOL_SCAN_OK_STRONG")
            self.get_logger().info(
                f"Pose publiziert nach {self.icp_runs_done} Runs (starke ICP-Qualitaet)"
            )
            self.get_logger().info("Bereit fuer naechsten Enter-Scan")
            return

        # Frueher Abschluss: sobald mindestens 2 Runs durch sind und ein stabiles Paar vorliegt.
        if (
            self.stable_pair_found
            and self.stable_pair_pose is not None
            and self.icp_runs_done >= MIN_RUNS_BEFORE_EARLY_ACCEPT
            and self.stable_success_streak >= REQUIRED_STABLE_SUCCESS_STREAK
        ):
            self.scan_active = False
            p_rel, r_rel = self.stable_pair_pose
            self._publish_detected_tf(p_rel, r_rel)
            self._publish_pose(p_rel, r_rel)
            self._publish_status("TOOL_SCAN_OK_STABLE")
            self.get_logger().info(
                f"Pose publiziert nach {self.icp_runs_done} Runs (frueh akzeptiert)"
            )
            self.get_logger().info("Bereit fuer naechsten Enter-Scan")
            return

        if self.icp_runs_done < ICP_RUNS_PER_TRIGGER:
            return

        self.scan_active = False
        if self.stable_pair_found and self.stable_pair_pose is not None:
            p_rel, r_rel = self.stable_pair_pose
            self._publish_detected_tf(p_rel, r_rel)
            self._publish_pose(p_rel, r_rel)
            self._publish_status("TOOL_SCAN_OK_STABLE")
            self.get_logger().info("Pose publiziert (3 stabile erfolgreiche ICP-Runs)")
            self.get_logger().info("Bereit fuer naechsten Enter-Scan")
        else:
            self._publish_status("TOOL_SCAN_NOT_STABLE")
            self.get_logger().warn("Keine 3 stabilen erfolgreichen ICP-Runs in 5 ICP-Runs")
            self.get_logger().info("Bereit fuer naechsten Enter-Scan")

    def _publish_pose(self, p_rel: np.ndarray, r_rel: np.ndarray) -> None:
...
def main(args=None) -> None:
    ...
    node = D405ArucoThenToolStable5Node()
    try:
        rclpy.spin(node)
